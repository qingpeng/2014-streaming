...
sensitivity of 100\% and a specificity of 67.5\% (Table~\ref{tab:a}).
...
and a specificity of 88.8\% vs 88.7\% for streaming vs two-pass,
...
specificity of 97.9\% for streaming and 98.0\% two-pass (compare
...
coverage, allowing existing spectral error correction approaches to be
...
correct high-coverage portions of mRNAseq and shotgun metagenome data
sets, which yielded promising results (Table~\ref{tab:quake_mrna}).
...
sublinear memory/less than two pass approach with little alteration of
results.  While data set characteristics affect the algorithmic
performance (see ``Time and space considerations'', above), the
algorithm performs {\em more efficiently} with {\em more} data, which
...
effect on data set coverage.  Moreover, trimming generally eliminates
far less of the data set than digital normalization.  This may make
