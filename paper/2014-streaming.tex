...
% @CTB why are specificities so low? Many reads have multiple errors.
...
(high coverage only) & -1.20\%    &                  &             & -0.66\% \\
Mock metagenome & 0.31\%         &                  &             & 0.28\% \\
(high coverage only) & 0.16\%          &    @@              &             & 0.07\%       \\
\hline
\end{tabular}
...
Coverage cutoff      & 20                 & 20 \\
\hline
Total reads          & --         & 103.2m \\
Total bp             & --      & 10.4 Gbp \\
High-coverage reads  & --         & 91.9m \\
Number of passes     & --               & 1.43 \\
\% reads trim        & \%               & 10.1\% \\
\% bp trim           & \%            & 4.03\% \\
Pre-trim error rate  & 1.88\%             & 0.27\% \\
Post-trim error rate & 1.29\%             & 0.15\% \\
\hline
\end{tabular}
